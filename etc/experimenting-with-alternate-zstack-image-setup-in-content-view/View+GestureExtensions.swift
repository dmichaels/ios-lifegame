...
public extension View {
    func onSmartGesture(dragThreshold: Int = 2, // pixels/points
...
private struct SmartGesture: ViewModifier
{
    ...
    private func _normalizePoint(_ point: CGPoint) -> CGPoint? {
        ...
        // normalized point, if normalizedPoint is indeed specified.
        //
            ...
            DragGesture(minimumDistance: 0)
                .onChanged { value in
                    if (self.onDragStrict) {
                        //
                        // We think it is probably best not to check the validity of the
                        // location/point, via ignorePoint, once the dragging has started;
                        // but can set onDragStrict to true to get the more strict behavior;
                        // so by default ignorePoint only applies to onTap/onDoubleTap/onLongTap.
                        // And also FYI note that _dragStart here has the non-normalize point.
                        //
